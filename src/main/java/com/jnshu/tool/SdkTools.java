...
@Autowired
    ...
    private boolean qiNiuFileToAliyunReal(String bucketname, String prefix, int limit, String delimiter, String prefixHttp) {
        // 获取七牛OSS中文件列表
        BucketManager.FileListIterator fileListIterator = sdkTools.qnYun.getObjectList(bucketname, prefix, limit, delimiter);
        if (fileListIterator != null) {
            ...
            while (fileListIterator.hasNext()) {
                ...
                for (FileInfo item : items) {
                    ...
                    sdkTools.aliSdk.updateFile(null,inputStream, item.key, item.mimeType);
                }
            }
            return true;
        }
